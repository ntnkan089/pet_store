...
const NavBar = () => {
    ...
    const links = [
        {id:1, link: 'Adoption', site: 'adoption-form'},
        {id:2, link: 'Login', site: 'login'},
        {id:3, link: 'Sign up', site: 'register'},
  ...
  return (
    ...
    <div className="flex justify-between items-center w-full h-20 text-black bg-white px-4 z-10 ">
        <div>
        ...
        <ul className="hidden md:flex">
            {
                links.map((link) => (
                    <li key = {link.id} className="px-4 cursor-pointer capitalize font-medium text-gray-500 hover:scale-105 duration-200">
                        <Link to={link.site} duration={500}>{link.link}</Link>
                    </li>))
            }
        </ul>
        ...
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-500 text-gray-500 ">
            {
                links.map((link) => (
                    <li key = {link.id} className="px-3 cursor-pointer capitalize py-5 text-4xl">
                        <Link to={link.site} onClick={() => setNav(!nav)} smooth duration={500}>{link.link}</Link>
                    </li>))
            }
        </ul>
        }
